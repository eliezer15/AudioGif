...
video_repo = VideoRepository(env['DB_FILE'], env['SEARCH_FILE'])
...
def on_message_reply(update: Update, context: CallbackContext):
    ...
    output_message = video_repo.save_favorite(update.message.reply_to_message.video, update.message.from_user)
    update.message.reply_text(output_message)
...
def on_mention(update: Update, context: CallbackContext):
        ...
        for video in videos:
            results.append(
                build_inline_query(video.title, video.playable_video_id)
            )
